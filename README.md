...
Динамические настройки сохраняются в `localStorage` и восстанавливаются при загрузке, **если файл конфигурации не менялся** (см. `configHash` ниже):

- `tabs` — вкладки и ссылки, изменённые через интерфейс;
- `background` — текущий фон;
- `customBackgrounds` — список кастомных фонов;
- `customBanners` — список кастомных баннеров вкладок;
...
- Если хеши совпадают (файл не менялся) — динамические настройки берутся из `localStorage` (сохраняются правки через интерфейс).
- Если хеши не совпадают (файл изменён или подставлен новый) — динамические настройки берутся из `userconfig.js` (применяются изменения в файле).

Правило распространяется на все динамические настройки: `tabs`, `background`, `customBackgrounds`, `customBanners`, `openLastVisitedTab`. Таким образом, замена `userconfig.js` (импорт) переписывает в том числе фон и кастомные изображения, а правки через интерфейс сохраняются до следующего изменения файла.
...
Так как содержимое `userconfig.js` изменилось, его `configHash` не совпадёт с сохранённым, и все динамические настройки (вкладки, фон, кастомные изображения) будут взяты из нового файла — очищать `localStorage` не требуется.
